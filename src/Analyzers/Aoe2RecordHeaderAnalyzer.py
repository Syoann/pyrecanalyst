...
class Aoe2RecordHeaderAnalyzer(Analyzer):
    def run(self):
        ...
        if version == 1000.0:
            map_name = self.read_aoe2_record_string()
        ...
        for key in ('trading_enabled', 'team_bonuses_disabled', 'randomize_positions',
            ...
            data[key] = ord(self.header[self.position]) != 0
            self.position += 1
        ...
        if version >= 1004.0:
            ...
            separator = struct.pack('cccc', '\xA3', '\x5F', '\x02', '\x00')
        ...
        data['fog_of_war_enabled'] = ord(self.header[self.position])
        self.position += 1
        data['cheat_notifications_enabled'] = ord(self.header[self.position])
        self.position += 1
        data['colored_chat_enabled'] = ord(self.header[self.position])
        self.position += 1

        # Separator.
        self.position += 4

        data['is_ranked'] = ord(self.header[self.position])
        self.position += 1
        data['allow_spectators'] = ord(self.header[self.position])
        self.position += 1
    ...
    def read_players1004(self, version, num_players):
        ...
        for i in range(0, 8):
            ...
            ai_civ_name_index = ord(self.header[self.position])
            self.position += 1
            unknown_name = None
            if version >= 1005.0:
